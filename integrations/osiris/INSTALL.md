...
| `routes/geohazards-route.ts` | `src/app/api/geohazards/route.ts` — volcanoes + tsunamis (no key): USGS elevated volcano alert levels, **coordinates joined from the Smithsonian GVP WFS by volcano number** (USGS omits them), plus NOAA Tsunami Warning Center message feeds (National + Pacific atom). 5-min cache |
| `routes/ofac-route.ts` | `src/app/api/ofac/route.ts` — OFAC sanctions actions (no key): scrapes Treasury's `recent-actions` page for dated designations / de-listings / general licenses, classified by kind. Hourly cache |
| `routes/hackernews-route.ts` | `src/app/api/hackernews/route.ts` — Hacker News front page (Algolia HN Search API, no key): top stories by points — a tech-attention pulse. 5-min cache |
| `routes/balloons-route.ts` | `src/app/api/balloons/route.ts` — weather balloons / radiosondes (SondeHub, no key): latest telemetry frame per sonde → `{balloons:[{callsign,lat,lng,altitude,speed,verticalRate,temperature,type,status,color}]}` for the map's balloon layer. **Do not set Accept-Encoding** (SondeHub gzips; undici auto-decompresses). Was a 404 (frontend fetched a non-existent route); now live |
| `routes/radiation-route.ts` | `src/app/api/radiation/route.ts` — radiation monitors (Safecast, no key): most recent reading per device, CPM→nSv/h normalized, thresholded to NORMAL/WARNING/DANGER → `{stations:[…]}` for the map's radiation layer. Was a 404; now live |
| `routes/wmo-alerts-route.ts` | `src/app/api/wmo-alerts/route.ts` — WMO SWIC global severe-weather CAP alerts (no key): official alerts from member met agencies worldwide, filtered to Severe/Extreme, grouped by country+event (country from the CAP OID's ISO-3166 numeric code). 10-min cache. *2026-08-20 World Monitor cherry-pick* |
| `routes/mil-flights-route.ts` | `src/app/api/mil-flights/route.ts` — live military aircraft worldwide (adsb.lol `/v2/mil`, no key, ODbL): sanitized positions; the engine clusters them into 10° posture cells. Density tracks receiver coverage — US/EU-heavy. 2-min cache. *2026-08-20 World Monitor cherry-pick* |
| `routes/ransomware-route.ts` | `src/app/api/ransomware/route.ts` — ransomware.live v2 recently-claimed victims (no key, free non-commercial): group/sector/country tape; descriptions are untrusted leak-site text, truncated hard. 30-min cache. *2026-08-20 World Monitor cherry-pick* |
| `FeedsWindow.tsx` | `src/components/FeedsWindow.tsx` — the floating **Signals** window (tool strip, Rss icon): every *non-geographic* feed that can't live on the globe — Kalshi odds, power grid, wastewater, climate dials, OFAC, Hacker News, space weather, US airspace, Wikipedia attention — each one-lined with its latest value + source link. Rendered via the `'feeds'` FloatingWindow kind in `page.tsx` |
...
- **Layer audit / no-hidden-feeds (2026-07):**
  - `src/components/OsirisMap.tsx` — added a **volcano map layer**: `'volcanoes'` source, `volcano-glow/dots/label` layers (circle colored by aviation code red>orange>yellow), a click popup, and a `setGeo('volcanoes', …)` effect that plots `data.geohazards.volcanoes` points that have coords. Removed the dead `war-alerts-targets` / `war-alerts-lines` sources (declared but never drawn).
  - `src/app/page.tsx` — `activeLayers.volcanoes` (default on) + a `/api/geohazards → { geohazards }` fetch; the **Comms & Intel Lattice** (`sdk_sea/air/naval`) now defaults **off** and the submarine-cable data fetch is re-gated from the removed `cables` key to `sdk_sea`; removed dead `war_alerts` + `cables` state keys; added the `'feeds'` FloatingWindow kind + a **Signals** tool-strip button (Rss icon → `FeedsWindow`).
  - `src/components/LayerPanel.tsx` — a **Volcanoes (USGS)** toggle in the Natural Hazards group, and a new **COMMS & INTEL LATTICE** group exposing the previously-untoggle-able `sdk_sea` (Submarine Cables) / `sdk_air` (Air Corridors) / `sdk_naval` (Naval Intel Mesh).
